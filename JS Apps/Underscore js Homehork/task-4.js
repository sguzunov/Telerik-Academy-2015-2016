...
function solve() {
    function drawHeader(groupName) {
        console.log(new Array(groupName.length + 2).join('-'));
        console.log(groupName + ':');
        console.log(new Array(groupName.length + 2).join('-'));
    }

    return function(animals) {
        var groupedAndSortedBySpeicesDescending = _.chain(animals)
            .sortBy('species')
            .reverse()
            .groupBy('species')
            .value();

        _.each(groupedAndSortedBySpeicesDescending, function(group, groupName) {
            var sortedByLegs = _.chain(group)
                .sortBy('legsCount')
                .sortBy(sortedByLegs, 'name')
                .value();

            drawHeader(groupName);            
